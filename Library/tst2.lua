...
local Lucide = loadstring(game:HttpGet("https://raw.githubusercontent.com/Utkaky/Main/refs/heads/main/ObsidianLib/Icons.lua"))()
...
function ResolveNotificationIcon(image)
	...
	local lucideIcon = Lucide.GetAsset(image)
	if lucideIcon then
		return {
			Url = lucideIcon.Url,
...
function OrionLib:MakeWindow(WindowConfig)
    ...
    local function RemoveOption(Option)
        if Dropdown.Buttons[Option] then
            ...
            if n then
                ...
                vgs.TS:Create(Dropdown.Buttons[Option], TweenInfo.new(.15, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {BackgroundColor3 = Color3.fromRGB(200, 80, 80)}):Play()
            else
    ...
    local function AddOption(Player)
        ...
        if not Dropdown.Buttons[Option] then
            ...
            if n then
                ...
                vgs.TS:Create(Dropdown.Buttons[Option], TweenInfo.new(.15, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {BackgroundColor3 = OrionLib.Themes[OrionLib.SelectedTheme]["Divider"]}):Play()
            end
        end
    end
    ...
    local function FilterOptions(text)
        ...
        for name, button in pairs(Dropdown.Buttons) do
            local visible = string.find(string.lower(name), text, 1, true)
            if not visible and button:FindFirstChild("Subtitle") then
                visible = string.find(string.lower(button.Subtitle.Text), text, 1, true)
            end
    ...
    function Dropdown:UpdVis()
        ...
        local visCount = math.min(Options, MaxElems)
        local totH = self.Toggled and (38 + (visCount * 45) + totSrchH) or 38
    ...
    function Dropdown:Set(Value, Once)
        ...
        if Dropdown.Multi then
            if not n then
                ...
                vgs.TS:Create(Dropdown.Buttons[Value], TweenInfo.new(.15, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {BackgroundTransparency = 0.5}):Play()
                vgs.TS:Create(Dropdown.Buttons[Value].Title, TweenInfo.new(.15, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {TextTransparency = 0}):Play()
            elseif n and not Once then
                ...
                if Dropdown.Buttons[Value].State.Visible then
                    ...
                    vgs.TS:Create(Dropdown.Buttons[Value], TweenInfo.new(.15, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {BackgroundTransparency = 1}):Play()
                    vgs.TS:Create(Dropdown.Buttons[Value].Title, TweenInfo.new(.15, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {TextTransparency = 0.4}):Play()
                end
            end
            ...
            DdFrame.Header.Selected.Text = TruncateText(DdFrame.Header.Selected, text, math.max(DdFrame.Header.Selected.AbsoluteSize.X - 250, 50))
            ...
            if Dropdown.Buttons[Value] then
                ...
                DdFrame.Header.Selected.Text = TruncateText(DdFrame.Header.Selected, tostring(Dropdown.Value), math.max(DdFrame.Header.Selected.AbsoluteSize.X - 250, 50))
            else
                Dropdown.Value = nil
                DdFrame.Header.Selected.Text = ""
            end
    ...
    if DropdownConfig.Searchable then
        ...
        AddConnection(SrchBox:GetPropertyChangedSignal("Text"), function()
            ...
            FilterOptions(Dropdown.srchTxt)
        end)
    end
